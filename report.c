...
void report_production_history(unsigned int produced[],
                               unsigned int blockchainXConsumed[],
                               unsigned int blockchainYConsumed[])
{
  int p; /* array index */
  int total;
  ...
  /* show number consumed by BlockchainX */
  printf("%s consumed ", consumerNames[BlockchainX]);
  total = 0;
  for (p = 0; p < RequestTypeN; p++)
  {
    if (p > 0)
    ...
    total += blockchainXConsumed[p];
    printf("%d %s", blockchainXConsumed[p], producerAbbrevs[p]);
  }
  printf(" = %d total\n", total);

  /* show number consumed by BlockchainY */
  printf("%s consumed ", consumerNames[BlockchainY]);
  total = 0;
  for (p = 0; p < RequestTypeN; p++)
  {
    if (p > 0)
    ...
    total += blockchainYConsumed[p];
    printf("%d %s", blockchainYConsumed[p], producerAbbrevs[p]);
  }
  printf(" = %d total\n", total);
